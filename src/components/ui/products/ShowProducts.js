...
const ShowProducts = props => {
  ...
  const itemAction = (id) => {
    fetch(`http://3.92.201.85:8999/product/${id}`, {
      method: 'DELETE'
    })
    .then(response => {
      if(response.ok) {
        ...
        <Row className="lists">
          ...
          {products.map((item, idx) => (
            <Col md={3} key={idx}>
              <Card>
                <Card.Body>
                  <Card.Text> {item.product_name || item.product_name}</Card.Text>
                  <Card.Text>Price: {item.price}</Card.Text>
                  {/* <Card.Text>Brand: {item.brand.name}</Card.Text> */}
                  ...
                  <div className="onhover-icons">
                    ...
                    >
                      <span>
                        <Link to={`/admin/${path}/edit-product/${item._id}`}> <FontAwesomeIcon icon={faPenNib} /></Link>
                      </span>
                    </OverlayTrigger>
          ...
          <tbody>
            {products.map((item, idx) => (
              <tr key={idx}>
                ...
                <td>{item.product_cateogry}</td>
                <td>{item.maker}</td>
                <td>{item.caliber}</td>
                <td>{item.bullet_weight}</td>
      ...
      <Container fluid>
        <Row>
          <Col md={10}>
            ...
            <h3 align="center" className="mt-5">All Products</h3>

            {getListLayout()}

            <ActionModal display={modal} itemAction={() => itemAction(id)} closeModel={() => setModal(false)} success={true} message="Are you sure ?" redirectTo={`/admin/${path}/products`} type="confirm" />
                  ...
                  <ul>
                    {props.brands.map((item, idx) => (
                      <li
                        ...
                        onClick={() => props.filterByBrand(item)}
                      >
                        <p>
                    ...
                    <li><a href="#reset" onClick={() => props.filterByBrand()}>Reset filters</a></li>
                  </ul>
                </div>
              </Col>
            </Row>
