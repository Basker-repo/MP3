...
const ShowVendors = props => {
  ...
  const handleSearch = e => {
    props.searchedVendors(e.target.value)
  }
  ...
  const itemAction = (id) => {
    fetch(`http://3.92.201.85:8999/vendor/${id}`, {
      method: 'DELETE'
    })
    .then(response => {
      if(response.ok) {
        ...
        <Row className="lists">
          { vendors.map((item, idx) => (
            <Col md={3} key={idx}>
              <Card>
                <Card.Body>
                  <Card.Text>{item.vendor_name}</Card.Text>
                  <Card.Text>priority: {item.priority}</Card.Text>
                  <Card.Text>url: {item.site_url}</Card.Text>
                  ...
                  <div className="onhover-icons">
                    ...
                    >
                      <span>
                        <Link to={`/admin/${path}/edit-vendor/${item._id}`}> <FontAwesomeIcon icon={faPenNib} /></Link>
                      </span>
                    </OverlayTrigger>
      ...
      <Container fluid>
        <Row>
          <Col md={10}>
            ...
            <h3 align="center" className="mt-5">All vendors</h3>
